...
;(function(window) {
	...
	var BookingEngine = function (target,schedules,bookings,idb,owner,rooms,roomNames,nf,firsthour,lasthour,time) {
		...
		this.dayNamesRus = ['понедельник', 'вторник', 'среда', 'четверг', 'пятница', 'суббота', 'воскресенье'];
	...
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	
		...
		if (this.nf == "1") {
			...
			*/
			...
			e4 = function() {
				BookingEngine.div = $("<div/>",{class:"futureB",html:"<div class=\"price\">Кликните чтобы забронировать</div>"}).css("height","70px").css("position","absolute").appendTo(this).hide();
			...
			e6 = function(e) {
				...
				$(document).off('mouseleave','.booked');
		     	...
		     	BookingEngine.div = $("<div/>",{class:"NFbooking",html:"<div class=\"price\"><a>Далее</a></div>"}).css("height","70px").css("position","absolute").css("z-index","7").appendTo(this).show();
		     	BookingEngine.div.time = $("<div/>",{class:"timesch"}).prependTo(BookingEngine.div);
	...
	*/
	BookingEngine.prototype.showConfirmation = function () {
		...
		$("<tr />",{html: "<td>Дата:</td><td>"+this.dayNamesRus[this.dayOfWeek]+", "+this.bookingData['date']+"</td>"}).appendTo(table);
		$("<tr />",{html: "<td>Время:</td><td>"+this.formatTime(this.bookingData['start'])+" - "+this.formatTime(this.bookingData['end'])+"</td>"}).appendTo(table);
		$("<tr />",{html: "<td>Комната:</td><td>"+this.roomNames[this.bookingData['room']]+"</td>"}).appendTo(table);
			
		if (window.user !== undefined) {
			$("<tr />",{html: "<td>Имя:</td><td>"+window.user.info.name+" "+window.user.info.lastname+"</td>"}).appendTo(table);
		} else if (this.owner) {
			$("<tr />",{html: "<td>Имя:</td><td><input type=\"text\" name=\"basebooking_name\"></input></td>"}).appendTo(table);
		}

		$("<tr />",{html: "<td>Группа:</td><td><input type=\"text\" name=\"basebooking_band\">"+""+"</input></td>"}).appendTo(table);
		$("<tr />",{html: "<td>Телефон:</td><td><input type=\"text\" name=\"basebooking_phone\">"+""+"</input></td>"}).appendTo(table);
		...
		if (this.nf == "1") {
			if (window.page.equipment[this.bookingData.room - 1].price !== "") {
				...
				$("<div />",{class:"priceInfo",html:"<b>Цена</b><br />"+"Информация по ценам на данную комнату еще не доступна"}).appendTo(dial2);
			}
		} else {
			dial2.html("<br />"+this.bookingData['price']+" руб");
		}
		$("<div />",{id: "exdial", class : "exbutton"}).appendTo(dial).show();

		if (window.user !== undefined || this.owner) {
			$("<div />",{class: "button bookingButton", id:"bookingButton", text:"Забронировать"}).appendTo(dial2);
		} else {
	...
	*/
	BookingEngine.prototype.showBooking  = function (data) {
		...
		dial1.html("<span>Просмотр бронирования</span>");
		$("<div />",{class: "space"}).appendTo(dial1);
		
		$("<tr />",{html: "<td>Дата:</td><td>"+this.dayNamesRus[this.dayOfWeek]+", "+data['date']+"</td>"}).appendTo(table);
		$("<tr />",{html: "<td>Время:</td><td>"+this.formatTime(data['start'])+" - "+this.formatTime(data['end'])+"</td>"}).appendTo(table);
		
		$("<tr />",{html: "<td>Комната:</td><td>"+this.roomNames[data['room']]+"</td>"}).appendTo(table);
		if (data['vkid'] != 0) {
			$("<tr />",{html: "<td>Музыкант:</td><td><a href=\"http://www.vk.com/id"+data['vkid']+"\">"+data['name']+" "+data['lastname']+"</a></td>"}).appendTo(table);
		} else {
			$("<tr />",{html: "<td>Музыкант:</td><td>"+data['name']+"</td>"}).appendTo(table);
		}
		
		$("<tr />",{html: "<td>Группа:</td><td>"+data['band']+"</td>"}).appendTo(table);
		$("<tr />",{html: "<td>Телефон:</td><td>"+data['phone']+"</td>"}).appendTo(table);

		table.appendTo(dial1);
		dial2.html("<br />"+data['price']+" руб");
		$("<div />",{id: "exdial", class : "exbutton"}).appendTo(dial).show();
		$("<div />",{id: "delete_booking", class : "button bookingButton", html:"Отменить"}).appendTo(dial2);
	...
	*/
	...
	BookingEngine.prototype.showSchedules = function () {
		...
		if (window.today > datenum) {
			this.message(1,"Вы выбрали день до сегодня!");
			setTimeout(function () {
	...
	*/
	BookingEngine.prototype.createFTime = function (data,pointer,roomNumber,style) {
		...
		if (this.owner && booked > -1) {
			...
			html = "<a>Группа: "+bookingPointer.band+"<a/><br/><a>Подробнее</a>";
	...
	*/
	...
	BookingEngine.prototype.appendScheduleHeader = function () {
		var header = "<div id=\"prevday\"></div><div id=\"nextday\"></div>Расписание на "+this.dayNamesRus[this.dayOfWeek]+", "+this.bookingData.date;
	...
	*/
	BookingEngine.prototype.showSchedule = function () {
		...
		if (this.rooms > 4) {
			$("<div />",{id: "nextArrow",text:"Следующие комнаты"}).prependTo(container);
			$("<div />",{id: "prevArrow",text:"Предыдущие комнаты"}).prependTo(container);
		} 
		...
		$.ajax({
			...
			success: function(data) {
				...
				if (data[1] === "debt" && basebooking.bookingData.vkid !== 0) {
					data[1] = "Долг в "+basebooking.bookingData.price+ "добавлен музыканту.";

				} else if (data[1] === "debt" && basebooking.bookingData.vkid === 0 ) {
					data[1] = "Репетиция успешно отменена";
				}
